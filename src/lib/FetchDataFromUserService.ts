...
// function to fetch all user data by user id from an array of items
export const fetchAllUsers = async <T extends { authorId: string; id: string }>(
  data: T[]
  ...
  await Promise.all(
    data.reverse().map(async (row) => {
      try {
        ...
        console.error(`Failed to fetch user for id=${row.id}`, err);
        usersMap[row.id] = {
          id: 'unknown',
          firstName: 'Unknown',
          lastName: '',
...
export const getAllFollowersOfUser = async (userId: string) => {
  ...
  try {
    ...
    GlobalNotifier(
      'An error occurred while trying to fetch followers.',
      'error'
    );
  }
};
...
export const getAllUsersAUserIsFollowing = async (userId: string) => {
  ...
  try {
    ...
    GlobalNotifier(
      'An error occurred while trying to fetch following users.',
      'error'
    );
  }
};
