...
const NavTabsFavoris = () => {
  ...
  const activeTab =
    pathname === '/u/favorites/blog'
      ? 'Blog'
      : pathname === '/u/favorites/podcast'
      ? 'Podcast'
      : '';
  ...
  const handleNavClick = (navName: 'Blog' | 'Podcast') => {
    ...
    if (navName === 'Podcast') {
      router.push(`/u/favorites/podcast`);
    } else if (navName === 'Blog') {
      router.push(`/u/favorites/blog`);
    }
  };
