...
import PodcastActionButton from './PodcastActionButton';
...
interface PodcastProfileCardInterface {
  post: PodcastInterface[];
  userData: GetUser;
}

const PodcastProfileCard: React.FC<PodcastProfileCardInterface> = ({
  post,
  userData,
}) => {
      ...
      {post?.length > 0 ? (
        <div className="bg-white rounded-lg w-full shadow-md ">
          {[...post]
            ...
            .map((post) => (
              <div
                className="cursor-pointer p-4 flex flex-col gap-3 w-full bg-white border-b border-grey-300 hover:bg-gray-100 trannsition"
                key={post.id}
              >
                <div onClick={() => handleUrl(post.status, post.id)}>
                  <div className="flex w-full justify-between items-start">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <PodcastActionButton podcast={post} />
                    </div>
                  </div>
